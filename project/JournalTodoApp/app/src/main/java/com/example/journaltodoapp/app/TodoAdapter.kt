...
class TodoAdapter(
    private var todoList: List<TodoTask>?,
    private val onItemClicked: (TodoTask) -> Unit,
    private val onItemChecked: (TodoTask, Boolean) -> Unit
) : RecyclerView.Adapter<TodoAdapter.TodoTaskViewHolder>() {
    ...
    override fun onBindViewHolder(holder: TodoTaskViewHolder, position: Int) {
        ...
        holder.checkbox.isChecked = todo?.isCompleted ?: false;
        if (todo != null) {
            holder.checkbox.setOnCheckedChangeListener { _, isChecked ->
                onItemChecked(todo, isChecked)
            }
